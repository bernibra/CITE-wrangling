...
select_relevant_files <- function(filenames, ftype, info){
...
load_db_idx <- function(paths, info, ftype="protein"){
...
load_db <- function(paths, ids, info, ftype="protein", rmfile=TRUE){
  ...
  # load each dataset
  lapply(datasets, function(x) load_db_idx(paths=paths[[x]], info=info[[ids[[x]]$id]], ftype=ftype))
